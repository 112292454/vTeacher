...
func GetPostIDsInOrder(p *entity.ParamPostList) ([]string, error) {
	// 从redis获取id
	// 1.根据用户请求中携带的order参数确定要查询的redis key
	key := KeyPostTimeZSet            // 默认是时间
	if p.Order == entity.OrderScore { // 按照分数请求
...
*
  ...
  - @Param orderKey:按照分数或时间排序
    ...
    *
*/
func GetCommunityPostIDsInOrder(p *entity.ParamPostList) ([]string, error) {
	// 1.根据用户请求中携带的order参数确定要查询的redis key
